#!/bin/bash -l
#SBATCH --job-name=ragtime2-encode
#SBATCH --output=logs/ragtime2-encode.out.%a
#SBATCH --error=logs/ragtime2-encode.err.%a
...
for model_dir in "${MODEL_DIRS[@]}"; do
    output_dir=${HOME}/scratch/ragtime2/${model_dir##*/}
    mkdir -p $output_dir

    for SHARD_ID in 0 1 2 3 4;do
        echo Encoding ragtime2 corpus $SHARD_ID
        python -m tevatron.retriever.driver.encode \
            ...
            --dataset_path ${HOME}/scratch/ragtime2/${LANG}.processed.jsonl.gz \
            --encode_output_path $output_dir/corpus_emb.${LANG}-${SHARD_ID}.pkl \
            --dataset_shard_index ${SHARD_ID} \
            --dataset_number_of_shards 5
    done

    # topic_path=/home/dju/trec2026/data/ragtime2/ragtime25_main_all.jsonl 
    # if [[ $LANG -eq "rus" ]]; then
